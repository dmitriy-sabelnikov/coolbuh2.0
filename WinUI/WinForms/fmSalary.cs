...
namespace WinUI.WinForms
{
    public partial class fmSalary : Form
    {
        ...
        private List<v_Salary> _vSalaries = new List<v_Salary>(); //кэш
        ...
        private void InitGridSalary()
        {
            ...
            dgvSalary.AddSortGrid<v_Salary>("Salary_Id");
            dgvSalary.AddSearchGrid();
            dgvSalary.AddStatusStripRow(statusStripRow, true);
        }
        ...
        private bool LoadDataToGridSalary(int salaryId, int depId, DateTime datBeg, DateTime datEnd)
        {
            ...
            BindingSource source = new BindingSource();
            _vSalaries = GetViewSalaries(_salaries, _persCards, _refDeps);
            source.DataSource = _vSalaries;
        ...
        private void UpdateSalaryResult()
        {
            StatTotDays.Text = "Всього дні: " + _vSalaries.Sum(rec => rec.Salary_Days).ToString();
            StatTotHours.Text = "Всього години: " + _vSalaries.Sum(rec => rec.Salary_Hours).ToString();
            StatTotBaseSm.Text = "Всього зарплата: " + _vSalaries.Sum(rec => rec.Salary_BaseSm).ToString() + " грн.";
            StatTotAllwncSm.Text = "Всього сума доплат: " + _vSalaries.Sum(
                rec => rec.Salary_PensSm + rec.Salary_ExpSm + rec.Salary_GradeSm + rec.Salary_OthSm).ToString() + " грн.";
            StatTotKTUSm.Text = "Всього сума згідно з КТУ: " + _vSalaries.Sum(rec => rec.Salary_KTUSm).ToString() + " грн.";
            StatTotResSm.Text = "Всього сума: " + _vSalaries.Sum(rec => rec.Salary_ResSm).ToString() + " грн.";
        }
        public fmSalary(Form owner)
        {
            InitializeComponent();
            this.SingleFormMode(owner);
            this.BaseFormStyle("Заробітна плата");
        }
        ...
        private void MenuItemDeps_Click(object sender, EventArgs e)
        {
            ...
            ShowDeps(MenuItemDeps.CheckState == CheckState.Checked);
        }
        ...
        private void ContextMenuDeps_Click(object sender, EventArgs e)
        {
            ...
            ShowDeps(ContextMenuDeps.CheckState == CheckState.Checked);
        }
        //--------------------------------------------------------------
    }
}
